...
def load_agent_cards(tool_context) -> List[str]:
    """
    로컬 agent_card 디렉토리에서 에이전트 카드 JSON을 읽어 state에 저장,
    에이전트 이름 리스트 반환
    """
    # 에이전트 카드 디렉토리 경로: ENV 우선, 기본값은 ./agent_card
    cards_dir = os.getenv("AGENT_CARD_DIR", os.path.join(os.getcwd(), "agent_card"))

    cards: dict[str, AgentCard] = {}

    if not os.path.isdir(cards_dir):
        logger.warning(f"에이전트 카드 디렉토리를 찾을 수 없습니다: {cards_dir}")
    ...
    for filename in os.listdir(cards_dir):
        ...
        try:
            ...
            # 이름 결정: name > url > 파일명(base)
            base_name = os.path.splitext(os.path.basename(file_path))[0]
            name = getattr(card, "name", None) or getattr(card, "url", None) or base_name
            cards[name] = card
        except Exception as e:
            logger.warning(f"에이전트 카드 로드 실패: {file_path} - {e}")
