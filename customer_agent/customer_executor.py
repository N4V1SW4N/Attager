...
class CustomerExecutor(AgentExecutor):
    ...
    async def _process_request(
        ...
        session_id = session_obj.id  # update with resolved session id
    ...
    async def execute(
        ...
        logger.debug("[customer] execute exiting")
    ...
    async def cancel(self, context: RequestContext, event_queue: EventQueue):
        ...
        if session_id in self._active_sessions:
            logger.info(
                f"Cancellation requested for active customer session: {session_id}"
            )
            self._active_sessions.discard(session_id)
        else:
            logger.debug(
                f"Cancellation requested for inactive customer session: {session_id}"
            )
...
def convert_a2a_part_to_genai(part: Part) -> types.Part:
    """Convert A2A Part → Google Gen AI Part."""
    part = part.root
...
def convert_genai_part_to_a2a(part: types.Part) -> Part:
    """Convert Google Gen AI Part → A2A Part."""
    if part.text:
